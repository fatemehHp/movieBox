...
const MovieDetails = () => {
   const{movieDetail} =useContext(SelectedMovie)
  if (!movieDetail) return <p className={styles.error}>فیلمی یافت نشد!</p>;

  return (
    <div className={styles.container}>
      <img className={styles.poster} src={movieDetail.Poster} alt={movieDetail.Title} />

      <div className={styles.details}>
        <h1 className={styles.title}>{movieDetail.Title} ({movieDetail.Year})</h1>
        ...
        <div className={styles.ratings}>
          <p className={styles.imdb}>⭐ imdbRating : {movieDetail.imdbRating}</p>
          <p className={styles.seasons}>📺  totalSeasons: {movieDetail.totalSeasons}</p>
